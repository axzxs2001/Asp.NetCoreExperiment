...
async Task<List<(string, double)>> SearchMemoryAsync(ISemanticTextMemory memory, string query)
{
    ...
    await foreach (var answer in memory.SearchAsync(
        ...
        withEmbeddings: true))
    {
        list.Add((answer.Metadata.Text, answer.Relevance));
        Console.WriteLine(answer.Metadata.Text);
    };
    if (list.Count == 0)
    {
        list.Add(("非常抱歉，我没有找到你要的问题！", 0));
    }
    return list;
}
void RunExampleAsync()
{
    ...
    var embeddingGenerator = new OpenAITextEmbeddingGenerationService(modelID, key);
    ...
    var chunks = paragraphs
        .ChunkByAggregate(
            seed: 0,
            aggregator: (tokenCount, paragraph) => tokenCount + GetTokenCount(modelID, paragraph),
    ...
    {
        var chunk = chunks[i];
        var embeddings = embeddingGenerator.GenerateEmbeddingsAsync(chunk).Result;
...
int GetTokenCount(string modelName, string text)
{
